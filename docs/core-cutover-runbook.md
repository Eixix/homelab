...
- If Cloudflare DDNS cannot read `/run/secrets/cloudflare_api_token`, confirm `DEPLOY_UID` and `DEPLOY_GID` match the owner of `/home/github/homelab/secrets/cloudflare_api_token`; production uses the `github` user, currently `1003:1003`, and the container must run with Docker's native `user:` setting.
